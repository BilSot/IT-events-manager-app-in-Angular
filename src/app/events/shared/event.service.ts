...
export class EventService {
  ...
  getEvent(id): IEventModel{
    return EVENTS.find(event => event.id === id);
  }

  saveEvent(eventData): void{
    /*eventData.id = 100;
    eventData.sessions = [];*/
  ...
  updateEvent(eventData): void{
    const eventIndex = EVENTS.findIndex(event => eventData.id === event.id);
  ...
  searchTerm(term: string): Observable<ISession[]>{
    ...
    EVENTS.forEach(event => {
      let matchedSessions = event.sessions.filter(session => session.name.toLowerCase().indexOf(searchTerm) > -1);
